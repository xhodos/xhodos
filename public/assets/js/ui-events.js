...
const dynamicChatBox = () => {
	...
	_chatSpace.style({height: `${innerHeight - navHeight}px`});
}

const chatOptions = (chatFriends) => {
	const _chatBodyMain = fx(chatBodyMain);
	...
	chatFriends.off().upon({
		click: function (e) {
			e.preventDefault();
			const _chatFriend = fx(e.currentTarget);
			const _chatMenu = fx(_chatFriend.parents(leftMenu));
			const _currentChatFriends = fx(chatFriends, _chatMenu);
			
			chatFriends.removeClass('active');
						...
						Object.keys(chatMessages.messages[chatMessage]).forEach((message, idxSub) => {
							...
							const contextMenu = () =>
								messageContent.type === 'from' ? `
									<div data-target="#${idx}${idxSub}" class="contextmenu from">
								...
								` : `
									<div data-target="#${idx}${idxSub}" class="contextmenu to">
			...
			if (chatFriendMessages) {
				...
				fx('.chat-message-content').off().upon('contextmenu', function (e) {
					...
					else if (_this.matchSelector('.to'))
						_this.children('.contextmenu').putClass('active');
				})
			} else
...
const menuToggleActions = () => {
	...
	_leftMenuTogglers.off().upon('click', function (e) {
		...
		if (!toggler.classes.contains('active')) {
			...
			_target.slideoutleft(0).then(element => {
				...
				element.putClass('active').slideinleft(transitionTime).then(() => {
					toggler.putClass('active');
					_leftMenuTogglers.disable(false);
				});
				_leftMenuTogglers.removeClass('active');
			});
		} else
			(isSmallScreen()) ? ((_target.classes.contains('opened')) && _target.slideinleft().then(element => {
	...
	chatOptions(_chatFriends);
}

export {isSmallScreen, dynamicChatBox, menuToggleActions}
