    ...
    toggleButtonLoadState(isLoading = true) {
      ...
      if (isLoading) {
        if (!button.prop("disabled") || !button.attrib("disabled"))
          loaderElement.length ? loaderElement.style("display") === "none" ? loaderElement.fadein().then(() => button.disable()) : button.disable() : button.disable();
      } else
        loaderElement.length ? loaderElement.style("display") !== "none" ? loaderElement.fadeout().then(() => button.disable(false)) : button.disable(false) : button.disable(false);
    }
    /**
    ...
    if (cancelButtonText || confirmButtonText) {
      ...
      modal.off("click").upon("click", function(e) {
        ...
        if (isCancel || isConfirm) {
          modal.hide(true);
          modal.off().upon("fx.modal.hide", (e2) => isCancel && typeof onCancel === "function" ? onCancel(e2, modal) : isConfirm && typeof onConfirm === "function" ? onConfirm(e2, modal) : null);
        }
      });
    } else {
      if (!cancelButtonText)
