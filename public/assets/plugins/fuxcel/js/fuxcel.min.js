...
(() => {
  // src/ts/fuxcel.ts
  var animations = ({ timeout = 300, iterations = 1, display = "unset" }) => {
    return {
      blink: {
        name: "blink",
        onBegin: {},
  ...
  var pushPropToWindow = (prop, value) => {
    window[prop] = value;
  };
  var fx = (selector, context = null) => new Fuxcel(selector, context);
  var isBool = (value) => {
  ...
  var FuxcelBase = class _FuxcelBase {
    length = 0;
    prev = { length: 0 };
    /**
    ...
    constructor(selector, context) {
      ...
      function init() {
        let selected;
        try {
          const _context = context && (isString(context) ? INSTANCE.#_toArray(document.querySelector(context)) : INSTANCE.#_toArray(context))[0];
    ...
    static get #_getCurrentScriptSrc() {
      ...
      for (const script of scripts) {
        ...
        if (_FuxcelBase.#_getCurrentScriptFilename && name) {
          if (_FuxcelBase.#_getCurrentScriptFilename.toLowerCase() === name.toLowerCase())
     ...
     */
    #_isIterable(element) {
      return !!_FuxcelBase.#_constructors.iterable.filter((value) => element.constructor.name.toLowerCase().includes("collection") || value === element.constructor.name.toLowerCase()).length || Array.isArray(element);
    }
    /**
    ...
    #_isHTMLElement(element) {
      return !!_FuxcelBase.#_constructors.html.filter((value) => element.constructor.name.toLowerCase().includes(value)).length;
    }
    /**
    ...
    static get guessPath() {
      const fullPath = _FuxcelBase.#_getCurrentScriptSrc;
  ...
  var Fuxcel = class _Fuxcel extends FuxcelBase {
    ...
    fadeout(timeout, iteration, display) {
      ...
      const animation = animations({ timeout, iterations: iteration, display }).fadeOut;
    ...
    fadein(timeout, iteration, display) {
      ...
      const animation = animations({ timeout, iterations: iteration, display }).fadeIn;
      return this.#_animate(animation);
    }
    /**
    ...
    slideindown(timeout, iteration, display) {
      ...
      const animation = animations({ timeout, iterations: iteration, display }).slideInDown;
      return this.#_animate(animation);
    }
    /**
    ...
    slideinup(timeout, iteration, display) {
      ...
      const animation = animations({ timeout, iterations: iteration, display }).slideInUp;
      return this.#_animate(animation);
    }
    /**
    ...
    slideoutdown(timeout, iteration, display) {
      ...
      const animation = animations({ timeout, iterations: iteration, display }).slideOutDown;
      return this.#_animate(animation);
    }
    /**
     *
    ...
    slideoutup(timeout, iteration, display) {
      ...
      const animation = animations({ timeout, iterations: iteration, display }).slideOutUp;
      return this.#_animate(animation);
    }
    /**
    ...
    slideinleft(timeout, iteration, display) {
      ...
      const animation = animations({ timeout, iterations: iteration, display }).slideInLeft;
      return this.#_animate(animation);
    }
    /**
    ...
    slideoutleft(timeout, iteration, display) {
      ...
      const animation = animations({ timeout, iterations: iteration, display }).slideOutLeft;
      return this.#_animate(animation);
    }
    /**
    ...
    slideinright(timeout, iteration, display) {
      ...
      const animation = animations({ timeout, iterations: iteration, display }).slideInRight;
      return this.#_animate(animation);
    }
    /**
    ...
    slideoutright(timeout, iteration, display) {
      ...
      const animation = animations({ timeout, iterations: iteration, display }).slideOutRight;
      return this.#_animate(animation);
    }
    /**
    ...
    blink(timeout, iteration, display) {
      ...
      const animation = animations({ timeout, iterations: iteration, display }).blink;
      return this.#_animate(animation);
    }
    /**
    ...
    zoomin(timeout, iteration, display) {
      ...
      const animation = animations({ timeout, iterations: iteration, display }).zoomIn;
      return this.#_animate(animation);
    }
    /**
    ...
    static get buttonLoaderClass() {
      return _Fuxcel.#_buttonLoaderClass;
    }
    /**
     * @return {string|null} The Plugin path.
     */
    static get path() {
      return `${_Fuxcel.#_pluginPath?.replace(/\/$/, "")}/..`;
    }
    /**
    ...
    static set buttonLoaderClass(token) {
      _Fuxcel.#_buttonLoaderClass = token;
    }
    /**
    ...
    replaceClass(oldToken, newToken) {
      const selected = this.toArray;
      selected.forEach((element) => element.classList.contains(oldToken) ? element.classList.replace(oldToken, newToken) : element.classList.add(newToken));
      return this;
    }
    /**
    ...
    attrib(name, value) {
      const selected = this.toArray;
      return name && !value && isString(name) ? selected[0].getAttribute(name) : isObject(name) ? this.#_setAttrib(name) : this.#_setAttrib(name, value);
    }
    /**
    ...
    dataAttrib(name, value) {
      ...
      return name && !value && isString(name) ? selected[0].dataset[formattedName] : isObject(name) ? this.#_setDataAttrib(name) : this.#_setDataAttrib(formattedName, value);
    }
    /**
    ...
    prop(name, value) {
      const selected = this.toArray;
      return name && !value && isString(name) ? selected[0][name] : isObject(name) ? this.#_setProp(name) : this.#_setProp(name, value);
    }
    /**
    ...
    style(name, value) {
      const selected = this.toArray;
      return name && !value && isString(name) ? window.getComputedStyle(selected[0]).getPropertyValue(name) : isObject(name) ? this.#_setStyle(name) : this.#_setStyle(name, value);
    }
    /**
     ...
     * @param handleError
     */
    handleFormSubmit({ uri = "", method = "get", data = null, dataType = "json", headers = null, beforeSend = null, timeout = 1e4, handleError = false } = {}) {
      const selected = this.toArray;
      let response;
      return new Promise(
        (resolve, reject) => selected.forEach((element) => {
              ...
              onComplete(xhr, status) {
                if (dataType === "json") {
                  ...
                  if (status > 199 && status < 300 || status === 308) {
                    ...
                    if (status === 419)
                      setTimeout(() => response.redirect ? location.href = response.redirect : location.reload(), 2e3);
                    else if (status === 422 || status === 500)
                      form.toggleFormSubmitButtonState(false).then(() => {
                        if (handleError && status === 422)
                          response.errors ? response.message ? form.renderValidationErrors(response.errors, response.message) : form.renderValidationErrors(response.errors) : response.message && form.renderValidationErrors({}, response.message);
                        else
...
'suffix'`;
    ...
    toggleButtonLoadState(isLoading = true) {
      return new Promise((resolve) => {
        ...
        const loaderElement = fx(_Fuxcel.buttonLoaderClass, button);
    ...
    upon(events, listener, option = true) {
      const selected = this.toArray;
      if (isObject(events) && listener === void 0)
        listener = true;
      selected.forEach((element) => {
  ...
  var FuxcelValidator = class _FuxcelValidator extends Fuxcel {
    #_fxValidatorConfig = _FuxcelValidator.defaultValidatorConfig;
    ...
    #_initValidateForms(forms) {
      forms.forEach((form, index) => {
        ...
        _FuxcelValidator.#_validatorErrorBag[formId] = {};
        _FuxcelValidator.#_validatorErrorCount[formId] = 0;
    ...
    #_initValidateStepForms(forms) {
      forms.forEach((form, index) => {
        ...
        if (!_currentForm.attrib("id"))
          _currentForm.attrib({ id: `current-form-${index}` });
        let formId = _currentForm.attrib("id"), formSteps = fx(`#${formId} ${_FuxcelValidator.stepsClass}`).formValidator;
        if (formSteps.length) {
          _FuxcelValidator.#_initSteps[index] = formId;
          _FuxcelValidator.#_validatorErrorBag[formId] = {};
          _FuxcelValidator.#_validatorErrorCount[formId] = {};
          configObject.config?.nativeValidation ? _currentForm.prop({ noValidate: false }) : _currentForm.prop({ noValidate: true });
          formSteps.toArray.forEach((step, index2) => {
            step.dataset.fxStep = `${index2 + 1}`;
            const stepIndex = parseInt(step.dataset.fxStep);
            const formGroups = fx(`.form-group`, step).formValidator;
            _FuxcelValidator.#_validatorErrorBag[formId][stepIndex] = {};
            _FuxcelValidator.#_validatorErrorCount[formId][stepIndex] = 0;
            if (formGroups.length) {
    ...
    #_manipulateErrorBag(MessageOrRemove) {
      ...
      if (isBool(MessageOrRemove) && MessageOrRemove)
        Object.keys(_FuxcelValidator.#_validatorErrorBag).length && (Object.values(_FuxcelValidator.#_initSteps).filter((value) => fieldAttribs.formId === value).length ? (
          // @ts-ignore
          Object.keys(_FuxcelValidator.#_validatorErrorBag[fieldAttribs.formId]).length && delete _FuxcelValidator.#_validatorErrorBag[fieldAttribs.formId][this.stepFromField][fieldAttribs.id]
        ) : (
          // @ts-ignore
          delete _FuxcelValidator.#_validatorErrorBag[fieldAttribs.formId][fieldAttribs.id]
        ));
      else {
        if (isString(MessageOrRemove))
          Object.keys(_FuxcelValidator.#_validatorErrorBag).length && (Object.values(_FuxcelValidator.#_initSteps).filter((value) => fieldAttribs.formId === value).length ? (
            ...
            // @ts-ignore
            _FuxcelValidator.#_validatorErrorBag[fieldAttribs.formId][fieldAttribs.id] = MessageOrRemove
          ));
      }
    ...
    #_manipulateErrorCount() {
      const fieldAttribs = this.fieldAttributes;
      Object.keys(_FuxcelValidator.#_validatorErrorCount).length && (Object.values(_FuxcelValidator.#_initSteps).filter((value) => fieldAttribs.formId === value).length ? (
        // @ts-ignore
        Object.keys(_FuxcelValidator.#_validatorErrorCount[fieldAttribs.formId]).length && (_FuxcelValidator.#_validatorErrorCount[fieldAttribs.formId][this.stepFromField] = Object.keys(_FuxcelValidator.#_validatorErrorBag[fieldAttribs.formId][this.stepFromField]).length)
      ) : (
        // @ts-ignore
        _FuxcelValidator.#_validatorErrorCount[fieldAttribs.formId] = Object.keys(_FuxcelValidator.#_validatorErrorBag[fieldAttribs.formId]).length
      ));
    }
    /**
      ...
      if (configObject.config?.useDefaultStyling) {
        ...
        if (configObject.config?.showPassword) {
          if (_fieldElement.attrib("type") && _fieldElement.attrib("type").toString().toLowerCase() === "password") {
            ...
            showPassword.setAttribute("alt", "\u{1F512}");
            hidePassword.setAttribute("alt", "\u{1F513}");
          ...
          if (Object.keys(configOptionObject).length)
            Object.keys(validatorConfigOptionObject).forEach((optionKey) => {
              if (optionKey in configOptionObject) {
                if (configOptionObject[optionKey] !== "" && configOptionObject[optionKey] !== null && configOptionObject[optionKey] !== void 0)
    ...
    #_validate(that, formGroup) {
      ...
      let refillRequired, passwordToggle = _FuxcelValidator.passwordTogglerIconClass, _inputElement = fx(inputElement, formGroup), _selectElement = fx(selectElement, formGroup), _textAreaElement = fx(textAreaElement, formGroup), _element = that.#_resetFuxcelObject(_inputElement.length ? _inputElement : _selectElement.length ? _selectElement : _textAreaElement), _passwordToggle = fx(passwordToggle, formGroup), showPasswordToggle = `#${formGroup.id} ${passwordToggle} > .fx-show-password-icon`, hidePasswordToggle = `#${formGroup.id} ${passwordToggle} > .fx-hide-password-icon`;
      ...
      _inputElement.length && _inputElement.upon({
        ...
        input: function() {
          ...
          const filterField = /* @__PURE__ */ new Set(["name", "username", "card_cvv", "card_number"]);
          const filterFieldType = /* @__PURE__ */ new Set(["date", "datetime", "email", "month"]);
                ...
                if (refillRequired && !_input.value()?.length) {
                  ...
                  if (!refillRequired && _input.value()?.length)
                    if (_input.attrib("type").toLowerCase() === "password")
                      _FuxcelValidator.#_toggleValidationIcons(hidePasswordToggle, showPasswordToggle);
                    else
                      _FuxcelValidator.#_toggleValidationIcons(showPasswordToggle, hidePasswordToggle);
                  else {
                    refillRequired = parseBool(_passwordToggle.dataAttrib("require-refill"));
      ...
      _selectElement.length && _selectElement.upon({
        ...
        change: function() {
          const _element2 = that.#_resetFuxcelObject(fx(this));
          _element2.canBeValidated && _element2.validateField();
        }
      });
    ...
    get canBeValidated() {
      const selected = this.toArray;
      return selected.length ? this.dataAttrib("fx-validate") ? parseBool(this.dataAttrib("fx-validate")) : this.parents(".form-group").length ? this.parents(".form-group").style("display") !== "none" : this.style("display") !== "none" : false;
    }
    /**
     * @return {object} The error bag for the current selected form.
     */
    get errorBag() {
      return this.length && this.isElement("form") && Object.keys(_FuxcelValidator.#_validatorErrorBag[this.attrib("id")]).length ? _FuxcelValidator.#_validatorErrorBag[this.attrib("id")] : null;
    }
    /**
     * @return {number} The error count for the current selected form.
     */
    get errorCount() {
      return this.length && this.isElement("form") && Object.keys(_FuxcelValidator.#_validatorErrorCount).length ? _FuxcelValidator.#_validatorErrorCount[this.attrib("id")] : 0;
    }
    /**
     ...
     */
    get stepFromField() {
      const stepDiv = this.parents(_FuxcelValidator.stepsClass);
      if (stepDiv.length)
    ...
    static get defaultValidatorConfig() {
      return _FuxcelValidator.#_defaultConfig;
    }
    /**
    ...
    static get stepsClass() {
      return _FuxcelValidator.#_stepsClass;
    }
    /**
    ...
    static set stepsClass(stepClass) {
      _FuxcelValidator.#_stepsClass = stepClass;
    }
    /**
    ...
    renderValidationErrors(errors = {}, messageOrCallbackFn = null, callbackFn = null) {
      ...
      if (selected.isElement("form")) {
        if (isObject(errors) && Object.keys(errors).length) {
          ...
          Object.keys(errors).forEach((elementId) => {
            ...
            if (elementId in fieldElements && isDefined(errors[elementId]))
              ...
              if (isString(errors[elementId]) && errors[elementId] !== void 0)
        ...
        console.warn("Non form element given.");
      typeof messageOrCallbackFn === "string" ? typeof callbackFn === "function" ? fx.modal({
        type: "error",
    ...
    showError(message = null) {
      ...
      this.#_fxValidatorConfig.config?.showIcons && _FuxcelValidator.#_toggleValidationIcons(validationProps.validIcon, validationProps.invalidIcon);
    ...
    showSuccess(message = null) {
      ...
      this.#_fxValidatorConfig.config?.showIcons && _FuxcelValidator.#_toggleValidationIcons(validationProps.invalidIcon, validationProps.validIcon);
    ...
    undoValidation(destroyValidation = false) {
      ...
      if (destroyValidation) {
        delete _FuxcelValidator.#_validatorErrorBag[fieldAttribs.formId][fieldAttribs.id];
        _FuxcelValidator.#_validatorErrorCount[fieldAttribs.formId] = Object.keys(_FuxcelValidator.#_validatorErrorCount[fieldAttribs.formId]).length;
      }
      ...
      !!fx(`${validationProps.validationIconField} > *`)?.length ? fx(`${validationProps.validationIconField} > *`).fadeout().then(() => fx(validationProps.validationField).formValidator.renderMessage(null)) : fx(validationProps.validationField).formValidator.renderMessage(null);
      return this;
    }
    /**
     ...
     */
    stepErrorBag(step) {
      return this.length && this.isElement("form") && Object.keys(_FuxcelValidator.#_validatorErrorBag[this.attrib("id")][step]).length ? _FuxcelValidator.#_validatorErrorBag[this.attrib("id")][step] : null;
    }
    /**
     ...
     */
    stepErrorCount(step) {
      return this.length && this.isElement("form") && Object.keys(_FuxcelValidator.#_validatorErrorCount[this.attrib("id")]).length ? _FuxcelValidator.#_validatorErrorCount[this.attrib("id")][step] : 0;
    }
    /**
    ...
    validateField(message = null, isError = false) {
      ...
      let fieldValue = target.value, minLength = parseInt(selected.attrib("minlength") ?? "0"), fieldName = fieldAttribs.id.toTitleCase(), finalMessage = minLength ? !isString(message) && fieldValue.length && fieldValue.length < minLength ? `The ${fieldName} field requires a minimum of ${minLength} characters.` : message : !isString(message) ? selected.isPasswordField ? fieldAttribs.id === configObject?.passwordConfirmId && configObject?.validatePassword ? !fieldValue.length || fieldValue !== fx(`#${configObject.passwordId}`).value() ? fx(`#${configObject.passwordId}`).value()?.length ? "Ensure passwords." : `The ${fieldName} field is required.` : null : !fieldValue.length ? `The ${fieldName} field is required.` : null : message : message;
    ...
    validateRegex(regExpOrFn, message) {
      const selected = this.toArray;
      const value = selected[0].value;
      regExpOrFn && typeof regExpOrFn === "function" ? regExpOrFn(this) : regExpOrFn && isString(message) ? value.length ? value.match(regExpOrFn) ? this.validateField() : this.validateField(message, true) : this.validateField() : console.error("Function `validateRegex()` expects 2 arguments.");
      return this;
    }
    /**
    ...
    validateUsername(regExp, customFormatEx = null) {
      ...
      return this.validateRegex(() => value.length ? value.length > minLength ? value.match(regExp) ? this.validateField() : this.validateField(`Invalid Username format: (${customFormatEx ?? "Username must start and end with an alphabet, and can only contain alphabets and underscores."})`) : this.validateField(customFormatEx ?? `The ${fieldName} requires a minimum of 3 characters.`) : this.toggleValidation());
    }
  };
  var FuxcelSteps = class _FuxcelSteps extends FuxcelValidator {
     ...
     */
    get context() {
      return new _FuxcelSteps(_FuxcelSteps.currentlySelected);
    }
    /**
    ...
    get formSteps() {
        ...
        this.toArray.forEach((form) => {
          if (fx(form).isElement("form")) {
            ...
            stepDivs.length && stepDivs.toArray.forEach((stepDiv) => {
              ...
              isString(step) && step !== void 0 && allSteps[form.id].push(step);
            });
          }
        });
        return allSteps;
      ...
      if (this.isElement("form")) {
        ...
        stepDivs.length && stepDivs.toArray.forEach((stepDiv) => {
          const step = stepDiv.dataset.fxStep;
          isString(step) && step !== void 0 && steps.push(step);
        });
      }
      return steps;
    }
    ...
    stepErrors(step = null) {
      ...
      if (step === null) {
        selected.forEach((element) => {
          const _element = new _FuxcelSteps(element);
          if (element.tagName && _element.isElement("form")) {
            errors[element.id] = {};
            const steps = _element.formSteps;
            if (steps.length) {
              steps.forEach((step2) => {
                errors[element.id][step2] = {
                  count: _element.stepErrorCount(step2),
                  errors: _element.stepErrorBag(step2)
                };
              });
            }
          }
  ...
  var FuxcelModal = class _FuxcelModal extends Fuxcel {
    #_isHiding = false;
    static #_modalTarget;
    static #_openModals = [];
    ...
    constructor(selector, context, autoActions = true) {
      super(selector, context);
      if (_FuxcelModal.modalTriggers.length) {
        _FuxcelModal.modalTriggers.off("click").upon("click", function(e) {
          e.preventDefault();
          const currentTrigger = fx(e.currentTarget);
          const modalAction = currentTrigger.dataAttrib("fx-action")?.toLowerCase() ?? "open";
          const modalTarget = currentTrigger.dataAttrib("fx-action")?.length ? currentTrigger.parents(".fx-modal").length ? currentTrigger.parents(".fx-modal") : null : fx(`#${currentTrigger.dataAttrib("fx-modal")}`);
          const triggerModal = () => {
            if (modalTarget) {
              _FuxcelModal.#_modalTarget = modalTarget;
              if (autoActions)
                if (modalAction === "close")
                  _FuxcelModal.#_modalTarget.modal.hide();
                else
                  _FuxcelModal.#_modalTarget.modal.toggle();
            }
          };
    ...
    static get currentModal() {
      return this.hasOpenModals ? _FuxcelModal.#_openModals[_FuxcelModal.#_openModals.length - 1] : null;
    }
    /**
     * @return {boolean} true if any modal is open. False otherwise.
     */
    static get hasOpenModals() {
      return !!_FuxcelModal.#_openModals.length;
    }
    /**
     * @return {Fuxcel}
     */
    ...
    hide(destroy = false) {
      ...
      if (!this.#_isHiding) {
        ...
        modalContent.fadeout(500).then(() => this.fadeout(500).then(() => {
          const index = _FuxcelModal.#_openModals.indexOf(this);
          if (index !== -1)
            _FuxcelModal.#_openModals.splice(index, 1);
          this[0].dispatchEvent(_FuxcelModal.fxModalHideEvent);
    ...
    show(escKey = true) {
      ...
      this.style({ pointerEvents: "none" }).fadein(300).then(() => modalContent.fadein(300).then(() => {
        _FuxcelModal.#_openModals.push(this);
          ...
          if (escKey)
            fx(document).upon("keyup", (e) => {
              ...
              if (key === "escape" || key === "esc") {
                if (_FuxcelModal.hasOpenModals)
                  _FuxcelModal.currentModal?.hide(true);
              }
            });
        }
        this[0].dispatchEvent(_FuxcelModal.fxModalShowEvent);
      }));
    }
    /**
  ...
  fx.fetch = function({ uri = "", method = "get", data = null, dataType = "json", headers = null, beforeSend = null, timeout = 1e4, onComplete = null, onError = null, onSuccess = null }) {
    let status, statusText, responseData;
    const controller = new AbortController();
    const timeoutID = setTimeout(() => controller.abort(), timeout);
    const allowedErrorStatuses = /* @__PURE__ */ new Set([301, 308, 401, 402, 419, 422, 423, 426, 451, 500, 511]);
    ...
    fetch(uri, {
      ...
      responseData.responseJSON = dataType === "json" && data2;
      responseData.responseText = dataType === "json" ? JSON.stringify(data2) : dataType === "text" && data2;
      ...
      isFunction(onError) && (error.name === "AbortError" ? onError(new TimeoutError(`\u23F0 Request timed out\r
Set Timeout:${timeout / 1e3}s`), 408, "timeout") : onError(error, status, statusText));
  ...
  fx.modal = function({ title = null, type = "success", content = "Alert Content", confirmButtonText = null, cancelButtonText = null, html = true, isStatic = false, closeOnConfirm = false, onConfirm = null, onCancel = null, onEsc = null } = {}) {
    ...
    const initialModal = FuxcelModal.init({ title, html, isStatic, content, id: "init", hasFooter: false });
    const modalBody = fx(".fx-modal-body", initialModal);
    const body = document.querySelector("body");
    alertIconPath = type === "success" ? `${Fuxcel.path}/images/ok-24.svg` : type === "error" ? `${Fuxcel.path}/images/cancel-24.svg` : `${Fuxcel.path}/images/warning-24.svg`;
    altAlertIcon = type === "success" ? "\u2705" : type === "error" ? "\u274C" : "\u26A0\uFE0F";
    const alertIcon = `<img src="${alertIconPath}" alt="${altAlertIcon}" class="fx-modal-alert-icon">`;
    const buttonsWrapper = (buttons2) => `<div class="fx-modal-alert-buttons">${buttons2}</div>`;
    const cancelButton = (content2) => `<button type="button" id="fx-modal-cancel" class="fx-btn fx-btn-error" data-fx-action="close" data-fx-target="modal">${content2}</button>`;
    const confirmButton = (content2) => `<button type="button" id="fx-modal-confirm" class="fx-btn fx-btn-primary" data-fx-target="modal" data-fx-modal="init">${content2}</button>`;
    const buttons = confirmButtonText && cancelButtonText ? cancelButton(cancelButtonText) + confirmButton(confirmButtonText) : confirmButtonText ? confirmButton(confirmButtonText) : cancelButtonText && cancelButton(cancelButtonText);
    ...
    fx(".fx-modal-alert-icon", initialModal).style({ visibility: "visible" }).fadein(2e3).then();
      ...
      modal.off("click").upon("click", function(e) {
        ...
        fx(".fx-modal-content", modal).hasFocus.then((focused) => {
          if (!focused && !parseBool(modal.dataAttrib("fx-static"))) {
            modal.hide(true);
            modal.off().upon("fx.modal.hide", (e2) => cancelButtonText && typeof onCancel === "function" ? onCancel(e2, modal) : !closeOnConfirm && typeof onConfirm === "function" ? onConfirm(e2, modal) : null);
          } else {
            if (isConfirm && !closeOnConfirm && typeof onConfirm === "function")
              ...
              if (isCancel || isConfirm || isCloseBtn) {
                modal.hide(true);
                modal.off().upon("fx.modal.hide", (e2) => (isCancel || isCloseBtn) && typeof onCancel === "function" ? onCancel(e2, modal) : isConfirm && typeof onConfirm === "function" ? onConfirm(e2, modal) : null);
              }
            }
          }
        });
  ...
  fx.passLuhnAlgo = (input) => {
    ...
    return input.split("").reverse().map(Number).map((value, index) => index % 2 !== 0 ? digitSum(value * 2) : 2).reduce((previous, current) => previous + current) % 10 === 0;
  };
  pushPropToWindow("fuxcel", Fuxcel);
  FuxcelModal.modalTriggers.length && new FuxcelModal("*");
})();
//# sourceMappingURL=fuxcel.min.js.map
